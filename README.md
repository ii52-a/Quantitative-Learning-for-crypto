...
- 完成demo版本的基础ui界面,准备进行策略回测的解耦，通过全局Config完成对接
...
### 2026/1/3 删除pycache缓存,计划升级策略模块
- 规划demo版本的仓位系统升级
- 基础的仓位管理系统和仓位修改

### 2026/1/15 仓位升级，考虑多种实际情况，部分平仓，全部平仓和反向开仓
- 仓位进行解耦设计，为多仓位策略提供优化接口
- 提供部分开仓，全部平仓和反向开仓指令转化
- TODO：思考真正的策略信号转化

### 2026/1/17 思考仓位管理架构，修改部分逻辑错误
- 对策略，仓位，数据进行再次思考，架构升级
- 发现部分逻辑错误和仓位管理问题，进行小规模修复

### 2026/1/18 对仓位管理进行测试和修改，修复大部分仓位变动问题
- 重构仓位管理模块和仓位模块的信号传递，重构 [策略-》仓位管理-》仓位] 的信号传递架构
- 使用多角度仓位进行管理的测试，对大多数问题进行修复

### 2026/1/21 优化仓位模块，尝试接入策略信号进行回测，修复逻辑问题，优化日志系统
- 修复方向和盈利反向问题，优化日志系统，更详细的仓位变动日志
- 将仓位模块接入策略模块进行CTA-macd30min回测测试，发现部分问题。
- TODO：修改API模块的k线组合逻辑，使得k线open为整点数而非错误点数
- TODO：写出一个含有CTA_MACD_FILTER的优化CTA策略
