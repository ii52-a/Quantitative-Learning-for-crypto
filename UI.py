...
class GetbackUi(QMainWindow):
    ...
    def _init_right_layout(self):
        ...
        self.strategy_combo.addItems(['CTA-MACD30min做多'])  # TODO:增加交互
    ...
    def start(self):
        ...
        setting_kline_num = int(self.h_combobox_kline_num.currentText())
        self.text_output.append("--- 参数读取成功 ---")
        self.text_output.append(f"交易对: {setting_trading_pair}")
        self.text_output.append(f"K线周期: {setting_kline},回测k线数量: {setting_kline_num}")

        # 禁用按钮
        self.button.setDisabled(True)

        # ApiWorker线程
        self.api_worker = ApiWorker(api=self.api,data_number= setting_kline_num,interval= TradeMapper.K_LINE_TYPE[setting_kline],symbol=setting_trading_pair)
        # 关联线程信号
        self.api_worker.data_ready.connect(self.on_data_ready)
        self.api_worker.start()

    def on_data_ready(self, data:pd.DataFrame | None) -> None:
        if not data.empty:
            self.text_output.append("数据加载完成")
            self.run_backtest(data)
        else:
            self.text_output.append("数据加载失败")
            self.button.setDisabled(False)

    def run_backtest(self, data:pd.DataFrame | None) -> None:
        try:
            back_test_setting = BackTestSetting(
...
class ApiWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.data_ready.emit(pd.DataFrame())  # 数据为空
